...
def define_word(word, dict_code):
    ...
    for row in results:
        if row.find(class_="FrWrd"):
            ...
            for em in row.td.find_all('em', class_='POS2'):
                em.extract()
            ...
            for em in row.td.next_sibling.next_sibling.find_all('em', class_='POS2'):
                em.extract()
            ...
            if row.find(class_="To2"):
                for em in row.td.next_sibling.find_all('em', class_='POS2'):
                    em.extract()
